...
sigma(lm(gentoo_body_mass[indexes_to_sample] ~ FL_cent[indexes_to_sample])) #333.281

summary(glm(gentoo_body_mass[indexes_to_sample] ~ FL_cent[indexes_to_sample], family = "gaussian")) 
sqrt(333229/5) #258.1585
...
ggplot(BM_FL_result, aes(x = FL, y = BM)) +
  geom_ribbon(aes(ymin = EstScatter_low, ymax = EstScatter_up), alpha = .3) +
  geom_point(alpha = .6) +
  geom_line(aes(x = FL, y = EstMean), col = "green", lwd = 1.2) +
...
##------------------4th example!
##---------------estimate rate parameter for a Poisson model of the number of Gentoo puppies
##---------------using a 'Bayesian' grid approximation


#first of all, although we know that the mean number of puppies generated by Gentoo adults does depend on body mass (as we simulated the data)
#(for the sake of simplicity) we here instead assume that the mean number of puppies is the same for all Gentoo penguins, regardless of their body mass
#which can be read (in terms of a parametric model) as "the number of puppies ~ Pois(lambda)", with lambda being the rate parameter

#second, as we did for estimating the population mean of BM of Gentoo penguins through brute force (see example 1),
#we set a sequence of possible values for the rate parameter of the Poisson model
#this sequence of values will constitute our 'grid' (although it's actually going to be a line, i.e. 1-dim)

#let's use the minmax value of the number of puppies in our sample to set a sequence of plausible rate parameters for the Pois distrbiution
Pois_param_seq <- seq(min(NumberPuppies_df$NumPuppies), max(NumberPuppies_df$NumPuppies), 0.01)

#let's avoid having a rate exactly equal to 0
Pois_param_seq[1] <- .1

#--set the prior
##let's assume (as a model for the rate parameter) that lambda is log-normally distributed
Pois_param_prior <- dnorm(log(Pois_param_seq), mean = 1.7, log = 0.7) #note that exp(1.7) is approx the mean number of puppies in the sample, and exp(0.7) is 2

#have a look at the implied prior
hist(rnorm(1000, mean = 1.7, sd = 0.7), xlab = "log(lambda)", main = "normal distribution of log(lambda)")
hist(exp(rnorm(1000, mean = 1.7, sd = 0.7)), xlab = "lambda", main = "log-normal distribution of lambda") #log-normal distribution of rate params

#--compute the likelihood associated with each lambda (given the data, i.e. the number of puppies)
Pois_param_l <- sapply(Pois_param_seq, function(i, num_pup) {
  lik_pois <- prod(dpois(num_pup, lambda = i))
  return(lik_pois)
}, num_pup = NumberPuppies_df$NumPuppies)


#--compute the marginal probability of the data (i.e. the number of puppies)
Pois_marginal_data <- sum(Pois_param_prior * Pois_param_l)

#--compute the posterior distribution for lambda
Pois_post <- (Pois_param_l * Pois_param_prior)/Pois_marginal_data

#check the posterior sums to 1
sum(Pois_post) #1

#have a look at the posterior distribution
ggplot(data.frame(PostDens = Pois_post, Param_val = Pois_param_seq), aes(x = Param_val, y = PostDens)) +
  geom_line(lwd = 0.8, alpha = .5) +
  geom_point() +
  ylab("Posterior density") + xlab("Lambda") +
  theme_pubr()

#now let's find the maximum a posteriori (MAP), i.e. the parameter value associated with the highest density
Pois_param_seq[which.max(Pois_post)] #5.59

#let's predict the distribution of the number of puppies given the posterior 
set.seed(495)
Sample_from_post <- sample(Pois_param_seq, size = 100000, replace = T, prob = Pois_post)

set.seed(906)
NumPuppies_from_post <- rpois(1000000, lambda = Sample_from_post)

Post_num_puppies <- ggplot(data.frame(NumPuppies = NumPuppies_from_post), aes(x = NumPuppies)) +
  geom_histogram() + xlab("Number of puppies") +
  theme_pubr()

#what if we sample the number of puppies setting lambda as the MAP
set.seed(756)
NumPuppies_from_map <- rpois(1000000, lambda = Pois_param_seq[which.max(Pois_post)])

Map_num_puppies <- ggplot(data.frame(NumPuppies = NumPuppies_from_map), aes(x = NumPuppies)) +
  geom_histogram() + xlab("Number of puppies") +
  theme_pubr()


Post_num_puppies | Map_num_puppies

var(NumPuppies_from_post); var(NumPuppies_from_map)
